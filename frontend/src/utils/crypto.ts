...
async function getPasswordKey(password: string): Promise<CryptoKey> {
  ...
  return await crypto.subtle.importKey('raw', enc.encode(password), { name: 'PBKDF2' }, false, [
    'deriveBits',
    'deriveKey',
  ]);
}
 ...
 */
export async function decryptData(encryptedBase64: string, password: string): Promise<unknown> {
  try {
    ...
    throw new Error('Decryption failed. Incorrect password or corrupted data.', { cause: e });
  }
}
