 ...
 */
export function getAccountContext(account: Account | undefined, t: TFunction): AccountContext {
  if (!account) {
  ...
  const country = rawCountryCode ? t(`countries.${rawCountryCode}`, rawCountryCode) : account.store;
