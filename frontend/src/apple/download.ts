  ...
  while (redirectAttempt <= 3) {
    ...
    if (dict.failureType) {
      ...
      switch (failureType) {
        ...
        case '9610':
          throw new DownloadError(i18n.t('errors.download.licenseRequired'), '9610');
        default: {
    ...
    const metadata = item.metadata as PlistDict | undefined;
    if (!metadata) {
    ...
    const sinfs: Sinf[] = [];
    const sinfData = item.sinfs as PlistDict[] | undefined;
    if (sinfData) {
    ...
    const metadataDict: PlistDict = { ...metadata };
