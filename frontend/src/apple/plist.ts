...
export function buildPlist(obj: PlistDict): string {
  return [
...
function buildNode(value: PlistValue): string {
  ...
  if (Array.isArray(value)) return `<array>${value.map(buildNode).join('')}</array>`;
  ...
  switch (node.nodeName) {
    case 'dict': {
      ...
      for (let i = 0; i < children.length; i += 2) {
        ...
        const value = i + 1 < children.length ? parseNode(children[i + 1]) : null;
        result[key] = value;
      }
      return result;
    }
