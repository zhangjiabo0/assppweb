...
export default function PackageDetail() {
  ...
  const { tasks, deleteDownload, pauseDownload, resumeDownload, hashToEmail } = useDownloads();
  ...
  async function handleShare(e: React.MouseEvent) {
    ...
    addToast(t('toast.msgShare', { appName, ...ctx }), 'success', t('toast.title.shareAcquired'));
    ...
    <PageContainer title={t('downloads.package.title')}>
      <div className="space-y-6">
        <div className="flex items-start gap-4">
          <AppIcon url={task.software.artworkUrl} name={task.software.name} size="lg" />
          <div className="flex-1">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">
              {task.software.name}
            </h2>
            <p className="text-gray-500 dark:text-gray-400">{task.software.artistName}</p>
        ...
        {task.error && <p className="text-sm text-red-500 dark:text-red-400">{task.error}</p>}
        ...
        <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-4">
          <dl className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <dd className="text-gray-900 dark:text-gray-200">{task.software.version}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-gray-500 dark:text-gray-400 flex-shrink-0">
                    ...
                    <div className="relative group flex items-center">
                      ...
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all duration-200 z-50 pointer-events-none">
                        <div className="bg-white p-2 rounded-lg shadow-xl border border-gray-200 flex flex-col items-center">
                          <QRCodeSVG value={installInfo.installUrl} size={128} className="mb-1" />
